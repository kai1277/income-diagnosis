...
> - DB に該当求人が 0 件の場合は MOCK_JOBS（静的データ）にフォールバック
> - Part 1・Part 2 はv2.0.0 と同一
...
flowchart TD
    ...
    ENG_REPLACE & ENG_KEEP --> CODES(["SuggestedJob[3]\n{ code, title, reason }\nPart 3 へ"])

    %% ── スタイル ──
    style OCC_IN fill:#0891b2,color:#fff,stroke:none
    style CODES fill:#059669,color:#fff,stroke:none
...
## Part 3 ── 求人表示ロジック（v2.1.0 NEW）

> Part 2 で選択した `SuggestedJob` の `code` 配列を使い、DB から実際の求人を引いて表示する。
> DB に該当求人が 0 件の場合は静的な MOCK_JOBS にフォールバックする。

```mermaid
flowchart TD
    CODES_IN(["SuggestedJob[3]\n{ code, title, reason }"])

    CODES_IN -->|code 列を抽出| RESULT_PAGE["/result 結果画面\nおすすめ職種を表示"]

    RESULT_PAGE -->|「この条件の求人を見る」クリック\nor「年収○○万円以上の求人」クリック| NAV

    NAV["navigate('/jobs?codes=code1,code2,code3')\n※両ボタンとも同じ codes を渡す"]

    NAV --> JOBS_PAGE["/jobs 求人スワイプ画面"]

    JOBS_PAGE -->|URL から codes を読み取り\nGET /api/user/jobs?codes=...| API["UserJobsController\nGET /api/user/jobs"]

    API --> SERVICE["UserJobsService\ngetJobsByOccupationCodes(codes)"]
    SERVICE --> REPO["UserJobsRepository\nfindByOccupationCodes(codes)"]

    REPO -->|Prisma クエリ| DB[("Supabase PostgreSQL")]

    DB_COND{"jobs.is_active = true\nAND expires_at IS NULL\nOR expires_at > NOW()\nAND job_occupation_types\n→ occupation_types.code\nIN (codes)"}
    DB --> DB_COND

    DB_COND -->|1件以上ヒット| TRANSFORM["UserJobDto[] に変換\nimage_url → imageUrl\nbadge_text → imageBadge\nsalary_text → incomeRange\njob_location → location\njob_types → jobTypes"]

    DB_COND -->|0件| FALLBACK["MOCK_JOBS\n（静的フォールバック）"]

    TRANSFORM --> SWIPE["JobCard スワイプUI\n右スワイプ: キープ\n左スワイプ: スキップ"]
    FALLBACK --> SWIPE

    SWIPE -->|affiliateUrl クリック\nGA4: job_link_click| EXT["外部求人サイト\n(A8.net アフィリエイト)"]
    SWIPE -->|全件確認完了| DONE["🎉 全ての求人を確認しました\n診断結果に戻るボタン"]

    %% ── スタイル ──
    style CODES_IN fill:#059669,color:#fff,stroke:none
    style NAV fill:#d97706,color:#fff,stroke:none
    style DB fill:#1e40af,color:#fff,stroke:none
    style DB_COND fill:#7c3aed,color:#fff,stroke:none
    style FALLBACK fill:#6b7280,color:#fff,stroke:none
    style SWIPE fill:#0891b2,color:#fff,stroke:none
    style EXT fill:#dc2626,color:#fff,stroke:none
